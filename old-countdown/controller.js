const TIMESTAMP_LOCALSTORAGE_KEY = "countdownTimestamp";

let countTo = 1634908500000 + new Date().getTimezoneOffset() * 60 * 1000;
...
function updateText(day, hour, minute, second) {
  ...
  document.querySelector(".hour.label").textContent =
    hour > 1 ? "Hours" : "Hour";
  document.querySelector(".minute.time").textContent = minute;
  document.querySelector(".minute.label").textContent =
    minute > 1 ? "Minutes" : "Minute";
  document.querySelector(".second.time").textContent = second;
  document.querySelector(".second.label").textContent =
...
function showTimeComplete() {
  ...
  document.querySelector(".time-complete").className = document
    .querySelector(".time-complete")
    .className.split(" ")
...
function showTimeText() {
  ...
  document.querySelector(".time-text").className = document
...
function getTimeLeft() {
  let left = Math.floor((countTo - Date.now()) / 1000);
...
function initialCompleteTime() {
  countTo = localStorage.getItem(TIMESTAMP_LOCALSTORAGE_KEY) || Date.now();
}
